...
router.get('/recent-calls', authenticateToken, requireAdmin, async (req, res) => {
  try {
    ...
    const formattedCalls = calls.map(call => ({
      callId: call._id,
      caller: {
        _id: call.userId?._id || call.userId,
        id: call.userId?._id || call.userId,
        name: call.userId?.name || 'Unknown',
        email: call.userId?.email || '',
        avatarUrl: call.userId?.avatarUrl || null
      },
      receiver: call.targetUserId ? {
        _id: call.targetUserId._id || call.targetUserId,
        id: call.targetUserId._id || call.targetUserId,
        name: call.targetUserId.name || 'Unknown',
        email: call.targetUserId.email || '',
        avatarUrl: call.targetUserId.avatarUrl || null
      } : null,
      callType: call.callType,
      duration: call.duration || 0,
      durationMinutes: Math.round(((call.duration || 0) / 60) * 100) / 100,
